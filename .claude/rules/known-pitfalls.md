...
- **新規 Color トークンを追加する時は既存 Primitive と照合する**
  - 症状: 新しい色が必要になった時、既存の `:root` を確認せず `--new-color: #xxxxxx` を追加すると、Primitive と値が重複するトークンが無秩序に増殖する。`DESIGN.md §2.11` で承認した完全重複ペアは 2-c 棚卸し時点のスナップショットのみで、追加時は同表と改定履歴の同時更新が必須
  - 対処: 追加候補の色を決めたら本セクション第 1 項（`:root` 全変数列挙必須化）を先に実行し、同値の Primitive（`--neutral-*` または `--accent-*`）が既存にあるかを確認する。既存にある場合は Semantic として `--{category}-{variant}` 命名で追加する（`category` = `text` / `bg` / `border` / 状態色）。既存にない場合はまず Primitive として追加し、次に Semantic として追加する。追加候補の色が既存 Primitive と RGB 各成分 ±16 以内の微差なら新規追加せず既存を流用する（`DESIGN.md §2.11.3` の禁止ルール）。真に別色（RGB 各成分 ±16 超）の場合のみ新規 Primitive を定義し、`DESIGN.md §2.11.2` の承認表と改定履歴を同時更新する
  - 前例: 2-c（2026-04-19）で DESIGN.md §2.11 に二層構造を明文化したのを契機に本ルールを制定
