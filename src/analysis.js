import { AI_ENABLED_KEY, ANALYSIS_LIMIT_KEY, ANALYSIS_TOTAL_KEY, ANALYSIS_CACHE_DAYS, ANALYSIS_DAILY_MAX, FIELD_TYPES, ROOF_TYPES, API_BASE } from "./constants.js";
...
export async function fetchPlayerAnalysis(name,m,isAdminMode,newIndicators,recentScores,playerName,env){
...
const res=await fetch(API_BASE+"/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({syncCode:localStorage.getItem("mk-sync-code")||"",playerName:pn,gameCount:gc,hitRate:m.missRate!=null?1-m.missRate:null,winRate:m.winRate??null,missRate:m.missRate??null,finishRate:m.finishRate??null,avgPts:m.avgPts??null,breakAvg:m.breakAvg??null,recAvg:m.recAvg??null,firstWinRate:m.firstWinRate??null,lastWinRate:m.lastWinRate??null,topScores:m.topScores||null,burstCount:newIndicators?.burstCount??null,scoreStdDev:newIndicators?.scoreStdDev!=null?Math.round(newIndicators.scoreStdDev*100)/100:null,zoromeHits:newIndicators?.zoromeHits??null,finishEfficiency:newIndicators?.finishEfficiency??null,recentScores:recentScores||null,level:lvl,levelSource:lvlSrc,environment:env?{fieldType:env.field?FIELD_TYPES.find(f=>f.value===env.field)?.label||env.field:null,roofType:env.roof?ROOF_TYPES.find(r=>r.value===env.roof)?.label||env.roof:null,weather:env.weather||null,temperature:env.temp??null,windSpeed:env.roof==="indoor"?null:(env.windSpeed??null),weatherNote:env.roof==="indoor"?"屋内のため天候・風速の影響なし":env.roof==="roof"?"屋根下のため雨・雪の直接影響なし（風は影響あり）":null}:null})});
