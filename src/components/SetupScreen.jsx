...
const go=()=>{let ft;if(mode==="manual"){ft=teams.slice(0,tc).map(t=>({...t,players:t.players.filter(p=>p.trim())}));if(!ft.every(t=>t.players.length>0))return;}else{if(!sp||!isSpValid())return;ft=sp;}if(windSensorEnabled&&!windSensorPiAddr.trim())return;const ord=Array.from({length:ft.length},(_,i)=>i);onStart(ft,ord,numGames,bestOf,dqEnd,saveToStats,courtCount,courtCount>=2&&allCourtData?{courtCount,courtTeamCounts,courtData:allCourtData}:null,selectedLocation,windSensorEnabled?windSensorPiAddr.trim():null);};
...
<input type="radio" name="loc" checked={!selectedLocation} onChange={()=>{setSelectedLocation(null);localStorage.removeItem("molkky_selected_location");}} style={{accentColor:"var(--blue-500)"}}/>
<span style={{fontSize:14,fontWeight:600,color:!selectedLocation?"var(--blue-500)":"var(--text-secondary)"}}>場所を選択しない</span>
</label>
...
<input type="radio" name="loc" checked={selectedLocation&&selectedLocation.id===loc.id} onChange={()=>{setSelectedLocation(loc);localStorage.setItem("molkky_selected_location",JSON.stringify(loc.id));}} style={{accentColor:"var(--blue-500)"}}/>
<span style={{flex:1}}>
...
const CHA={borderColor:"var(--blue-500)"};
...
const PIN={flex:1,border:"1px solid var(--neutral-200)",borderRadius:10,padding:"12px 14px",fontSize:20,outline:"none",background:"#fafafa"};
const TIN={flex:1,border:"none",borderBottom:"2px solid var(--neutral-200)",padding:"6px 4px",fontSize:22,fontWeight:700,outline:"none",background:"transparent"};
return(
...
<button onClick={()=>setCaDiscardStep(1)} style={{flex:"0 0 auto",padding:"10px 16px",border:"2px solid color-mix(in srgb, var(--danger) 40%, transparent)",borderRadius:10,background:"color-mix(in srgb, var(--danger) 10%, transparent)",color:"var(--danger)",fontSize:14,fontWeight:700,cursor:"pointer"}}>{"🗑"} 破棄</button>
</div></div>}
...
<input value={windSensorPiAddr} onChange={e=>{const v=e.target.value;setWindSensorPiAddr(v);setWindTestStatus(null);setWindTestResult(null);setWindTestError("");try{localStorage.setItem("windSensorPiAddress",v);}catch(ex){}if(onWindDebugPiAddress)onWindDebugPiAddress(v);}} placeholder="例: 192.168.1.100" style={{width:"100%",padding:"10px 14px",border:"1px solid var(--neutral-200)",borderRadius:10,fontSize:17,outline:"none",background:"#fafafa",boxSizing:"border-box"}}/>
...
{editMode&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}><button onClick={()=>{setEditMode(false);setExpandedDel(null);}} className="mk-btn-primary" style={{padding:"6px 18px",border:"none",borderRadius:8,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer"}}>Done</button></div>}
...
{editMode&&<button onClick={()=>{setEditMode(false);setExpandedDel(null);}} className="mk-btn-primary" style={{padding:"4px 14px",border:"none",borderRadius:8,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>Done</button>}</div>
...
{!okS&&okSReason&&<div style={{fontSize:13,fontWeight:700,color:"var(--danger)",textAlign:"center",marginBottom:4}}>{okSReason}</div>}
...
<WindMonitorModal isOpen={showWindMonitor} onClose={()=>setShowWindMonitor(false)} piAddress={windSensorPiAddr} windDebugEnabled={windDebugEnabled} onWindDebugLog={onWindDebugLog}/>
...
{showCourtOverview&&allCourtData&&<CourtOverview courtData={allCourtData} courtCount={courtCount} courtTeamCounts={courtTeamCounts} numGames={numGames} bestOf={bestOf} dqEnd={dqEnd} saveToStats={saveToStats} onStartGame={(teams,order)=>{setShowCourtOverview(false);onStart(teams,order,numGames,bestOf,dqEnd,saveToStats,courtCount,{courtCount,courtTeamCounts,courtData:allCourtData},selectedLocation,windSensorEnabled?windSensorPiAddr.trim():null);}} onBack={()=>{setShowCourtOverview(false);if(allCourtData){const allNames=[];for(let c=1;c<=courtCount;c++){const teams=allCourtData[c]||[];teams.forEach(t=>t.players.forEach(p=>{const nm=typeof p==="string"?p:(typeof p==="object"?(p.name||""):String(p));if(nm.trim())allNames.push(nm);}));}if(allNames.length>0)setMems(allNames);}}}/>}
