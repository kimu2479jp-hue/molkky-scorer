...
function *trimOldestStats(stats,maxGames){
...
stats[nm]=stats[nm].filter((*,i)=>!removeMap[nm].has(i));
...
<colgroup><col style={{width:rw}}/>{ordered.map(o=><React.Fragment key={o.idx}>{o.ap.map((*,pi)=><col key={pi} style={{width:cw}}/>)}<col style={{width:cw}}/></React.Fragment>)}</colgroup>
...
Array.from({length:showRows},(*,i)=>i+1).map(turn=>{
...
const[teams,setTeams]=useState(()=>{if(savedTeams){const base=savedTeams.map(t=>({name:t.name,players:t.players.length>0?t.players:[””]}));while(base.length<MAX_TEAMS)base.push({name:“チーム”+(base.length+1),players:[””]});return base.slice(0,MAX_TEAMS);}return Array.from({length:MAX_TEAMS},(*,i)=>({name:“チーム”+(i+1),players:[””]}));});
...
const[syncCode,setSyncCode]=useState(()=>getSyncCode());const[syncStatus,setSyncStatus]=useState(””);const[showSync,setShowSync]=useState(false);
...
const rP=(ti,pi)=>setTeams(p=>p.map((t,i)=>i===ti&&t.players.length>1?{…t,players:t.players.filter((*,j)=>j!==pi)}:t));
const uM=(i,v)=>setMems(p=>p.map((m,j)=>j===i?v.slice(0,MAX_NAME):m));
const aM=()=>{if(mems.length<MAX_SHUF)setMems(p=>[…p,””]);};
const rM=i=>{if(mems.length>2)setMems(p=>p.filter((*,j)=>j!==i));};
const doShuf=()=>{const names=mems.filter(m=>m.trim());if(names.length<tc)return;const s=shuf(names);const r=Array.from({length:tc},(*,i)=>({name:“チーム”+(i+1),players:[]}));s.forEach((n,i)=>r[i%tc].players.push(n));setSp(r);};
const okM=teams.slice(0,tc).every(t=>t.name.trim()&&t.players.some(p=>p.trim()));const okS=mems.filter(m=>m.trim()).length>=tc;
const go=()=>{let ft;if(mode===“manual”)ft=teams.slice(0,tc).map(t=>({…t,players:t.players.filter(p=>p.trim())}));else{if(!sp)return;ft=sp;}let ord=Array.from({length:ft.length},(*,i)=>i);if(om===“random”)ord=shuf(ord);onStart(ft,ord,numGames,bestOf,dqEnd,saveToStats);};
...
<div onClick={()=>setShowSync(p=>!p)} style={{padding:“14px 16px”,border:“2px solid rgba(255,255,255,0.15)”,borderRadius:12,background:“rgba(255,255,255,0.04)”,cursor:“pointer”,display:“flex”,alignItems:“center”,justifyContent:“space-between”}}>
<span style={{color:“rgba(255,255,255,0.7)”,fontSize:15,fontWeight:700}}>☁️ クラウド同期 {syncCode?”(設定済)”:”(未設定)”}</span>
<span style={{color:“rgba(255,255,255,0.4)”,fontSize:14}}>{showSync?“▲”:“▼”}</span>
</div>
{showSync&&(<div style={{marginTop:8,padding:“14px 16px”,background:“rgba(255,255,255,0.96)”,borderRadius:12}}>
<div style={{fontSize:13,color:”#888”,marginBottom:8}}>同期コードを設定すると、iPad・iPhoneなど複数端末でスタッツとお気に入りを共有できます。</div>
...
<button onClick={()=>{
if(syncCode.length<3){setSyncStatus(“❌ 3文字以上入力してください”);return;}
setSyncCodeLS(syncCode);setSyncStatus(“⏳ 同期中…”);
pullFromServer().then(r=>{
if(r.merged)setSyncStatus(“✅ 同期完了”+(r.added>0?” (+”+r.added+“件)”:””));
else if(r.reason===“error”)setSyncStatus(“❌ “+r.error);
...
{syncCode&&syncCode.length>=3&&(<button onClick={()=>{
setSyncStatus(“⏳ アップロード中…”);setSyncCodeLS(syncCode);
pushToServer().then(r=>{setSyncStatus(r.ok?“✅ アップロード完了”:“❌ “+r.error);});
}} style={{width:“100%”,padding:“10px”,border:“1px solid #ddd”,borderRadius:8,background:”#f8f9fa”,color:”#555”,fontSize:14,fontWeight:600,cursor:“pointer”,marginBottom:8}}>📤 手動アップロード</button>)}
{syncStatus&&<div style={{fontSize:14,color:syncStatus.startsWith(“✅”)?”#22b566”:syncStatus.startsWith(“❌”)?”#c0392b”:”#2b7de9”,fontWeight:600}}>{syncStatus}</div>}
<div style={{fontSize:12,color:”#bbb”,marginTop:6}}>同じコードを全端末で設定してください。試合終了時に自動同期されます。</div>
</div>)}
</div>
...
<div style={{display:“flex”,alignItems:“center”,gap:12}}><span style={{fontSize:32,fontWeight:900,color:teamColor}}>{teamName}</span><span style={{fontSize:36,fontWeight:900,color:”#fff”}}>{teamScore}点</span>{playerName&&<span style={{fontSize:18,fontWeight:600,color:“rgba(255,255,255,0.6)”}}>▶{playerName}</span>}<div style={{display:“flex”,gap:4}}>{Array.from({length:MF},(*,j)=>(<span key={j} style={{width:18,height:18,borderRadius:9,background:j<fails?”#e74c3c”:“rgba(255,255,255,0.2)”}}/>))}</div></div>
...
{fails>0&&<div style={{display:“flex”,gap:5,alignItems:“center”}}>{Array.from({length:MF},(*,j)=>(<span key={j} style={{width:26,height:26,borderRadius:13,display:“inline-flex”,alignItems:“center”,justifyContent:“center”,fontSize:14,fontWeight:900,background:j<fails?”#e74c3c”:”#ddd”,color:j<fails?”#fff”:”#999”}}>{j<fails?“✕”:””}</span>))}</div>}
</div>
...
*analysisPending.add(key);setLoading(true);setError(null);
let active=true;
(async()=>{try{
...
if(!res.ok){let errMsg=“API “+res.status;try{const raw=await res.text();try{const ej=JSON.parse(raw);if(ej.error)errMsg=typeof ej.error===“string”?ej.error:ej.error.message||errMsg;}catch(*){if(raw)errMsg+=”: “+raw.slice(0,120);}}catch(_2){} _analysisPending.delete(key);if(active){setError(errMsg);setLoading(false);}return;}
