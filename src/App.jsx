export default function App() {
  return (
    <div style={{ height: "100dvh", width: "100vw", color: "white" }}>
...
<colgroup>
<col style={{width:28}}/>
{ordered.map(o=><React.Fragment key={o.idx}>{o.ap.map((*,pi)=><col key={pi} style={{width:COL_W}}/>)}<col style={{width:COL_W}}/></React.Fragment>)}
</colgroup>
<thead>
<tr>
...
<tbody>
{Array.from({length:maxT},(*,i)=>i+1).map(turn=>{
...
const go=()=>{let ft;if(mode===“manual”)ft=teams.slice(0,tc).map(t=>({…t,players:t.players.filter(p=>p.trim())}));else{if(!sp)return;ft=sp;}let ord=Array.from({length:ft.length},(*,i)=>i);if(om===“random”)ord=shuf(ord);onStart(ft,ord,numGames,bestOf,dqEnd);};
