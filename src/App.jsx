...
const[selected,setSelected]=useState(()=>{const s=new Set();const showDev=(()=>{try{return localStorage.getItem("mk-dev-master")==="1";}catch(e){return false;}})();if(showDev){DEV_MASTER_LIST.forEach(n=>{if(favs.includes(n)&&!usedSet.has(n))s.add(n);});}return s;});
const showDevMaster=(()=>{try{return localStorage.getItem("mk-dev-master")==="1";}catch(e){return false;}})();
...
const grouped={};favs.forEach(name=>{const g=classify(name);if(!grouped[g])grouped[g]=[];const gs=stats[name]||[];grouped[g].push({name,games:gs.length});});
Object.values(grouped).forEach(arr=>arr.sort((a,b)=>b.games-a.games));
...
const[multiCourtShufData,setMultiCourtShufData]=useState(null);const[allCourtData,setAllCourtData]=useState(null);const[showSmartFav,setShowSmartFav]=useState(false);
...
const maxShufForCourt=courtCount*MAX_TEAMS*MAX_PL;
const aM=()=>{if(mems.length<maxShufForCourt)setMems(p=>[...p,""]);};
...
const total=allNames.length;const base=Math.floor(total/courtCount);const rem=total%courtCount;
const courtSizes={};for(let c=1;c<=courtCount;c++){const extraOrder=c===1?courtCount:c-1;courtSizes[c]=base+(extraOrder<=rem?1:0);}
...
const okM=teams.slice(0,tc).every(t=>t.name.trim()&&t.players.some(p=>p.trim()));const totalTeamsForAllCourts=courtCount===1?tc:Object.entries(courtTeamCounts).filter(([k])=>Number(k)<=courtCount).reduce((s,[,v])=>s+v,0);const okS=mems.filter(m=>m.trim()).length>=totalTeamsForAllCourts;
const go=()=>{if(courtCount>=2){window.alert("複数コート機能は準備中です");return;}let ft;if(mode==="manual")ft=teams.slice(0,tc).map(t=>({...t,players:t.players.filter(p=>p.trim())}));else{if(!sp)return;ft=sp;}const ord=Array.from({length:ft.length},(_,i)=>i);onStart(ft,ord,numGames,bestOf,dqEnd,saveToStats);};
