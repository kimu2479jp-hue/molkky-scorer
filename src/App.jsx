...
const isSpValid=()=>{if(!sp||!Array.isArray(sp))return false;const curNames=new Set(mems.filter(m=>m.trim()));const spNames=sp.flatMap(t=>t.players);if(spNames.length===0)return false;return spNames.every(n=>curNames.has(n));};
const go=()=>{if(courtCount>=2){window.alert("複数コート機能は準備中です");return;}let ft;if(mode==="manual")ft=teams.slice(0,tc).map(t=>({...t,players:t.players.filter(p=>p.trim())}));else{if(!sp||!isSpValid())return;ft=sp;}const ord=Array.from({length:ft.length},(_,i)=>i);onStart(ft,ord,numGames,bestOf,dqEnd,saveToStats);};
