...
function ensureBlink(){
  if(document.getElementById(BLINK_ID))return;
  const s=document.createElement("style");s.id=BLINK_ID;
  s.textContent=`@keyframes mk-blink{0%,100%{background:rgba(43,125,233,0.12)}50%{background:rgba(43,125,233,0.45)}}`;
...
/* Smart shuffle: avoid previous order if possible */
function smartShuf(arr,prevOrder,maxTries){
  const n=maxTries||20;
  if(arr.length<=1)return[...arr];
  for(let i=0;i<n;i++){const s=shuf(arr);if(!prevOrder||!arrEq(s,prevOrder))return s;}
  return shuf(arr);
}
...
/* ═══ Confirm — supports 3 buttons ═══ */
function Confirm({msg,sub,okLabel,cancelLabel,thirdLabel,onOk,onCancel,onThird}){
  return(<div style={SS.ov}><div style={{background:"#fff",borderRadius:20,padding:"32px 28px",maxWidth:440,width:"100%",textAlign:"center",boxShadow:"0 10px 36px rgba(0,0,0,0.25)"}}>
    ...
    <div style={{display:"flex",gap:10,flexDirection:"column"}}>
      ...
      {onThird&&<button onClick={onThird} style={{padding:"12px 0",border:"2px solid #ccc",borderRadius:12,background:"transparent",color:"#888",fontSize:15,fontWeight:600,cursor:"pointer"}}>{thirdLabel||"戻る"}</button>}
    </div>
  </div></div>);
}
...
function FavDropdown({favs,addF,rmF,onPick,usedNames}){
  ...
  return(
    <div ref={wrapRef} style={{position:"relative",display:"inline-block"}}>
      <button onClick={()=>{setOpen(!open);setDelTarget(null);}} style={{width:36,height:36,border:"1px solid #d0dff0",borderRadius:7,background:open?"#2b7de9":"#f0f6ff",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:open?"#fff":"#d9a83a"}}>⭐</button>
      {open&&(<div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:"#fff",border:"1px solid #ccc",borderRadius:10,boxShadow:"0 6px 20px rgba(0,0,0,0.18)",zIndex:50,minWidth:200,maxWidth:280,padding:8}}>
        {available.length===0&&<div style={{padding:10,textAlign:"center",color:"#aaa",fontSize:14}}>{favs.length===0?"登録なし":"全員配置済み"}</div>}
        <div style={{maxHeight:200,overflow:"auto"}}>{available.map(f=>(<div key={f}><button onPointerDown={()=>startLP(f)} onPointerUp={cancelLP} onPointerLeave={cancelLP} onClick={()=>{if(delTarget===f)setDelTarget(null);else{onPick(f);setOpen(false);}}} style={{width:"100%",padding:"10px 14px",border:"none",borderBottom:"1px solid #f0f0f0",background:delTarget===f?"#fde8e8":"transparent",fontSize:16,fontWeight:600,color:"#14365a",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between"}}><span>{f}</span>{delTarget===f&&<span onClick={e=>{e.stopPropagation();rmF(f);setDelTarget(null);}} style={{padding:"4px 10px",background:"#e74c3c",color:"#fff",borderRadius:5,fontSize:12,fontWeight:700}}>削除</span>}</button></div>))}</div>
        <div style={{borderTop:"1px solid #eee",paddingTop:8,marginTop:4}}><div style={{display:"flex",gap:4}}>
...
/* ═══ SCORE TABLE — with active cell blink & improved name styling ═══ */
function ScoreTable({teams,history,teamOrder,highlightLast,fontSize,colW,roundW,nameH,activeCell,forCapture}){
  ...
  /* Extend rows if active cell is beyond current max */
  ...
  return(
    <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed",borderSpacing:0}}>
      ...
      <thead>
        ...
        <tr>
          ...
          {ordered.map(o=><React.Fragment key={o.idx}>
            {o.ap.map((p,pi)=>(
              <th key={pi} style={{background:"linear-gradient(180deg,"+C[o.idx].bg+",#1e4a72)",color:C[o.idx].nm,fontWeight:800,fontSize:fs*0.78,textAlign:"center",verticalAlign:"top",borderLeft:pi===0?"3px solid "+C[o.idx].ac:"1px solid rgba(255,255,255,0.15)",position:forCapture?"static":"sticky",top:H1,zIndex:7,borderTop:"none",borderBottom:"2px solid #0d2a48",padding:"5px 2px",letterSpacing:1,textShadow:"0 1px 3px rgba(0,0,0,0.4)"}}>
                <span style={{...VT,fontSize:fs*0.78,maxHeight:nh,fontWeight:800}}>{p.name.slice(0,MAX_NAME)}</span>
              </th>
            ))}
            <th style={{background:"#0d2a48",color:"#ffd700",fontWeight:900,textAlign:"center",verticalAlign:"top",borderLeft:"1px solid rgba(255,255,255,0.2)",position:forCapture?"static":"sticky",top:H1,zIndex:7,borderTop:"none",borderBottom:"2px solid #0d2a48",padding:"5px 2px",textShadow:"0 1px 3px rgba(0,0,0,0.4)"}}>
              <span style={{...VT,fontSize:fs*0.78,maxHeight:nh,fontWeight:900}}>計</span>
            </th>
          </React.Fragment>)}
        </tr>
      </thead>
      <tbody>
        {showRows===0?(
          <tr><td colSpan={totalCols} style={{color:"#bbb",padding:24,fontSize:fs*0.8,textAlign:"center",borderBottom:"1px solid #eee"}}>スコアを入力してください</td></tr>
        ):(
                ...
                {ordered.map(o=>{
                  ...
                  return(<React.Fragment key={o.idx}>
                    {o.ap.map((p,pi)=>{
                      ...
                      const isActive=activeCell&&activeCell.turn===turn&&activeCell.teamIndex===o.idx&&activeCell.playerIndex===pi&&!e;
                      ...
                      const cellStyle={padding:"5px 3px",textAlign:"center",borderBottom:"1px solid #ddd",color:clr,fontWeight:fw,background:bg,borderLeft:pi===0?"3px solid "+C[o.idx].ac+"33":"1px solid #eee",fontSize:fs};
                      if(isActive)cellStyle.animation="mk-blink 1s ease-in-out infinite";
                      return <td key={pi} style={cellStyle}>{txt}</td>;
                    })}
                    <td style={{padding:"5px 3px",textAlign:"center",borderBottom:"1px solid #ddd",fontWeight:900,color:C[o.idx].tx,background:e?"#f0f3f8":"transparent",borderLeft:"2px solid #d0d0d0",fontSize:fs}}>{e?e.runningTotal:""}</td>
                  </React.Fragment>);
                })}
              </tr>
            );
...
function GameSheet({teams,history,currentTurn,teamOrder,activeCell}){
  ...
  useEffect(()=>{if(ref.current){const el=ref.current;setTimeout(()=>{el.scrollTop=el.scrollHeight;},50);}},[history.length,currentTurn]);
  return(<div ref={ref} style={{height:"100%",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
    <ScoreTable teams={teams} history={history} teamOrder={teamOrder} highlightLast={true} fontSize={20} colW={0} roundW={48} nameH={110} activeCell={activeCell}/>
  </div>);
}
...
function SetupScreen({onStart,savedTeams}){
  ...
  return(
    <div style={SS.setupW}>
      ...
      <div style={SS.setupB}>
        <div style={SS.sec}><label style={SS.sL}>入力モード</label><div style={{display:"flex",gap:6}}>{[["manual","✏️ 手動"],["shuffle","🎲 ランダム"]].map(([k,l])=>(<button key={k} onClick={()=>{setMode(k);setSp(null);}} style={{...SS.ch,...(mode===k?SS.chA:{})}}>{l}</button>))}</div></div>
        <div style={{display:"flex",gap:10,marginBottom:10}}>
          <div style={{flex:1}}><label style={SS.sL}>チーム数</label><div style={{display:"flex",gap:6}}>{[2,3,4].map(n=>(<button key={n} onClick={()=>{setTc(n);setSp(null);}} style={{...SS.ch,...(tc===n?SS.chA:{}),padding:"10px 0"}}>{n}</button>))}</div></div>
          <div style={{flex:1}}><label style={SS.sL}>投げ順</label><div style={{display:"flex",gap:6}}>{[["normal","通常"],["random","ランダム"]].map(([k,l])=>(<button key={k} onClick={()=>setOm(k)} style={{...SS.ch,...(om===k?SS.chA:{})}}>{l}</button>))}</div></div>
        </div>
        <div style={{display:"flex",gap:6,marginBottom:10}}>
          <div style={{flex:"1 1 0"}}><label style={SS.sL}>ゲーム数</label><select value={numGames} onChange={e=>setNumGames(+e.target.value)} style={SS.sel}>{Array.from({length:10},(_,i)=>i+1).map(n=><option key={n} value={n}>{n}ゲーム</option>)}</select></div>
          <div style={{flex:"1 1 0"}}><label style={SS.sL}>先取機能</label><select value={bestOf} onChange={e=>setBestOf(+e.target.value)} style={SS.sel}><option value={0}>なし</option>{Array.from({length:11},(_,i)=>i+2).map(n=><option key={n} value={n}>{n}先取</option>)}</select></div>
          <div style={{flex:"1 1 0"}}><label style={SS.sL}>失格時</label><select value={dqEnd?"end":"cont"} onChange={e=>setDqEnd(e.target.value==="end")} style={SS.sel}><option value="end">即終了</option><option value="cont">継続</option></select></div>
        </div>
        ...
        {mode==="manual"&&(<>{teams.slice(0,tc).map((team,ti)=>(
          <div key={ti} style={{...SS.card,borderLeft:"5px solid "+C[ti].ac}}>
            ...
            <div style={{paddingLeft:40}}>
              {team.players.map((p,pi)=>(<div key={pi} style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
        ...
        {mode==="shuffle"&&(<>
          <div style={SS.card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>{mems.map((m,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:3}}>
          ...
          {sp&&(<div style={{marginTop:6}}>
            {sp.map((t,ti)=>(<div key={ti} style={{...SS.card,borderLeft:"5px solid "+C[ti].ac,padding:"8px 12px",marginBottom:4}}>
...
function ScoreInput({dispatch,canUndo,teamName,teamScore,teamColor,playerName,fails,onConfirm,minimized,onToggleMin}){
  ...
  if(minimized){return(
    <div onClick={onToggleMin} style={{background:"#14365a",padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:18,fontWeight:900,color:teamColor}}>{teamName}</span>
        <span style={{fontSize:28,fontWeight:900,color:"#fff"}}>{teamScore}点</span>
        {playerName&&<span style={{fontSize:14,fontWeight:600,color:"rgba(255,255,255,0.6)"}}>▶{playerName}</span>}
        <div style={{display:"flex",gap:3}}>{Array.from({length:MF},(_,j)=>(<span key={j} style={{width:14,height:14,borderRadius:7,background:j<fails?"#e74c3c":"rgba(255,255,255,0.2)"}}/>))}</div>
      </div>
      <span style={{fontSize:24,color:"#fff",fontWeight:900}}>▲ 入力</span>
    </div>);}
  return(
    <div style={{background:"#fff",borderTop:"3px solid #dde1e6",padding:"10px 16px 14px",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
        <div style={{display:"flex",alignItems:"baseline",gap:10,flexWrap:"wrap"}}>
          ...
          {fails>0&&<div style={{display:"flex",gap:4,alignItems:"center"}}>{Array.from({length:MF},(_,j)=>(<span key={j} style={{width:22,height:22,borderRadius:11,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,background:j<fails?"#e74c3c":"#ddd",color:j<fails?"#fff":"#999"}}>{j<fails?"✕":""}</span>))}</div>}
        </div>
        <button onClick={onToggleMin} style={{padding:"6px 14px",border:"2px solid #14365a",borderRadius:8,background:"transparent",color:"#14365a",fontSize:16,fontWeight:800,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>▼ 最小化</button>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10,alignItems:"center"}}>
        {[[7,9,8],[5,11,12,6],[3,10,4],[1,2]].map((row,ri)=>(<div key={ri} style={{display:"flex",gap:8,justifyContent:"center"}}>{row.map(n=>{const isSel=sel===n;return(<button key={n} onClick={()=>setSel(sel===n?null:n)} style={{width:80,height:80,borderRadius:40,border:isSel?"4px solid #14365a":"3px solid #b0bec5",background:isSel?"#14365a":"#fff",color:isSel?"#fff":"#14365a",fontSize:34,fontWeight:900,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.1s",boxShadow:isSel?"0 4px 16px rgba(20,54,90,0.3)":"none"}}>{n}</button>);})}</div>))}
      </div>
...
function PlModal({teams,dispatch,onClose}){
  ...
  return(<div style={SS.ov} onClick={onClose}><div style={SS.mod} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:20,fontWeight:900,color:"#14365a"}}>👥 メンバー</h2><button style={SS.clsB} onClick={onClose}>✕</button></div>
    {teams.map((team,ti)=>(<div key={ti} style={{marginBottom:10}}>
      ...
      {team.players.map((p,pi)=>(<div key={pi} style={{display:"flex",alignItems:"center",padding:"5px 10px",background:p.active?"#f8f9fa":"#f0f0f0",borderRadius:6,marginBottom:3,opacity:p.active?1:0.4}}>
...
/* ═══ OrderPicker — always shows result, with re-shuffle ═══ */
function OrderPicker({teams,teamOrder,value,onChangeOrd,prevOrder}){
  ...
  return(<>
    <div style={{display:"flex",gap:5,marginBottom:6}}>{[["same","🔁同順"],["reverse","🔄裏"],["random","🎲ランダム"],["manual","✏️手動"]].map(([k,l])=>(<button key={k} onClick={()=>pick(k)} style={{flex:1,padding:"7px 0",border:"1px solid #ddd",borderRadius:7,background:value===k?"#14365a":"#fff",color:value===k?"#fff":"#14365a",fontSize:13,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",textAlign:"center"}}>{l}</button>))}</div>
    {disp&&(<div style={{background:"#f8f9fa",borderRadius:7,padding:6,marginBottom:6}}>
      {disp.map((ti,i)=>{const t=teams[ti];const ap=t?.players?t.players.filter(p=>typeof p==="object"?p.active:true):[];
        return(<div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",borderBottom:i<disp.length-1?"1px solid #eee":"none"}}>
          <span style={{fontSize:14,fontWeight:800,color:C[ti]?.ac||"#aaa",width:22,textAlign:"center"}}>{i+1}</span>
          <span style={{fontSize:15,fontWeight:700,color:C[ti]?.tx||"#333"}}>{t?.name||""}</span>
          <span style={{fontSize:12,color:"#888",marginLeft:2}}>{ap.map(p=>typeof p==="object"?p.name:p).join("・")}</span>
      ...
      {value==="random"&&<button onClick={doRand} style={{width:"100%",marginTop:4,padding:"7px 0",border:"2px dashed #2b7de9",borderRadius:6,background:"transparent",color:"#2b7de9",fontSize:13,fontWeight:700,cursor:"pointer"}}>🔄 再シャッフル</button>}
    </div>)}
  </>);
}

/* ═══ Image Save Helper ═══ */
function useImageSave(){
  ...
  const save=useCallback(async()=>{
    ...
    try{
      ...
      const el=captureRef.current;const w=el.scrollWidth;const h=el.scrollHeight;
      const clone=el.cloneNode(true);
      clone.style.position="absolute";clone.style.left="-9999px";clone.style.width=w+"px";
      document.body.appendChild(clone);
      const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><foreignObject width="100%" height="100%"><div xmlns="http://www.w3.org/1999/xhtml">${new XMLSerializer().serializeToString(clone)}</div></foreignObject></svg>`;
      document.body.removeChild(clone);
      const img=new Image();const blob=new Blob([svg],{type:"image/svg+xml"});const url=URL.createObjectURL(blob);
      img.onload=()=>{const c=document.createElement("canvas");c.width=w*2;c.height=h*2;const ctx=c.getContext("2d");ctx.scale(2,2);ctx.drawImage(img,0,0);URL.revokeObjectURL(url);
        c.toBlob(b=>{if(b){const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="molkky-score.png";a.click();URL.revokeObjectURL(a.href);}setSaving(false);},"image/png");
      };
      img.onerror=()=>{setSaving(false);alert("画像保存に失敗しました。スクリーンショットをご利用ください。");};
      img.src=url;
    }catch(e){setSaving(false);alert("画像保存に失敗しました。スクリーンショットをご利用ください。");}
  },[]);
  return{captureRef,saving,save};
}
...
function GameResult({teams,history,teamOrder,winner,gameWins,bestOf,numGames,gameNumber,onNext,onBack,onExtend}){
  ...
  const{captureRef,saving,save}=useImageSave();
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#f0f3f8",zIndex:100,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      {/* Top bar */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",background:"#14365a",flexShrink:0}}>
        <h2 style={{fontSize:22,fontWeight:900,color:"#fff",margin:0}}>🏆 Game {gameNumber} 結果</h2>
        <div style={{display:"flex",gap:8}}>
          <button onClick={save} disabled={saving} style={{padding:"8px 18px",border:"none",borderRadius:8,background:"#22b566",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",opacity:saving?0.5:1}}>{saving?"保存中...":"📸 画像保存"}</button>
        </div>
      </div>
      {/* Scrollable content */}
      <div style={{flex:1,overflow:"auto",WebkitOverflowScrolling:"touch",padding:"12px 16px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:10}}>
          <h2 style={{fontSize:28,fontWeight:900,color:C[winner]?.ac||"#14365a",margin:0}}>{teams[winner]?.name} 勝利！</h2>
          {isMatchOver&&<div style={{fontSize:20,fontWeight:900,color:"#22b566",marginTop:4}}>🎊 {teams[matchWin].name} {bestOf}先取達成！</div>}
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:12,flexWrap:"wrap"}}>{teamOrder.map(ti=>(<div key={ti} style={{textAlign:"center",padding:"10px 22px",borderRadius:14,background:ti===winner?C[ti].lt:"#fff",border:ti===winner?"3px solid "+C[ti].ac:"2px solid #e0e0e0"}}>
          <div style={{fontSize:15,fontWeight:700,color:C[ti].tx}}>{teams[ti].name}</div>
          <div style={{fontSize:40,fontWeight:900,color:C[ti].ac,lineHeight:1.1}}>{tw[ti]}</div>
          <div style={{fontSize:12,fontWeight:800,color:"#888"}}>勝</div>
        </div>))}</div>

        {/* Capturable area: score table + comments */}
        <div ref={captureRef} style={{background:"#fff",borderRadius:12,padding:12,marginBottom:12,border:"1px solid #ddd"}}>
          <div style={{fontSize:16,fontWeight:800,color:"#14365a",marginBottom:6}}>📋 Game {gameNumber} スコア表</div>
          ...
          <div style={{marginTop:8}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}><thead><tr style={{background:"#14365a",color:"#fff"}}><th style={{padding:"6px 8px",textAlign:"left"}}>チーム</th><th style={{padding:"6px"}}>最終</th><th style={{padding:"6px"}}>得点計</th><th style={{padding:"6px"}}>ミス</th><th style={{padding:"6px"}}>フォルト</th><th style={{padding:"6px"}}>ターン</th></tr></thead><tbody>{teamStats.map((ts,i)=>(<tr key={i} style={{background:ts.ti===winner?"#fffde6":"#fff",borderBottom:"1px solid #eee"}}><td style={{padding:"6px 8px",fontWeight:700,color:C[ts.ti].tx}}>{ts.ti===winner?"🏆":""}{ts.name}</td><td style={{padding:"6px",textAlign:"center",fontWeight:800,color:C[ts.ti].ac}}>{ts.final}</td><td style={{padding:"6px",textAlign:"center"}}>{ts.totalPts}</td><td style={{padding:"6px",textAlign:"center",color:"#bf6900"}}>{ts.misses}</td><td style={{padding:"6px",textAlign:"center",color:"#c0392b"}}>{ts.faults}</td><td style={{padding:"6px",textAlign:"center"}}>{ts.turns}</td></tr>))}</tbody></table>
          </div>
          {comments.length>0&&<div style={{marginTop:8,borderTop:"1px solid #eee",paddingTop:6}}>
            <div style={{fontSize:14,fontWeight:700,color:"#14365a",marginBottom:3}}>💬 コメント</div>
            {comments.map((c,i)=><div key={i} style={{padding:"4px 8px",background:"#f8f9fa",borderRadius:5,marginBottom:2,fontSize:13,color:"#444"}}>{c}</div>)}
          </div>}
        </div>

        {/* Comment input (outside capturable area) */}
        <div style={{marginBottom:12}}>
          <div style={{fontSize:15,fontWeight:800,color:"#14365a",marginBottom:4}}>💬 コメント追加</div>
          <div style={{display:"flex",gap:5}}>
            <input value={comment} onChange={e=>setComment(e.target.value)} placeholder="コメント..." onKeyDown={e=>{if(e.key==="Enter")addC();}} style={{flex:1,padding:"10px 12px",border:"1px solid #ddd",borderRadius:8,fontSize:15,outline:"none"}}/>
            <button onClick={addC} style={{padding:"10px 18px",border:"none",borderRadius:8,background:"#2b7de9",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",opacity:comment.trim()?1:0.3}}>追加</button>
          </div>
        </div>

        {/* Next game / extend */}
        {canContinue&&(<div style={{background:"#fff",borderRadius:12,padding:14,marginBottom:10,border:"1px solid #ddd"}}>
          <div style={{fontSize:16,fontWeight:800,color:"#14365a",marginBottom:6}}>次ゲームの投げ順</div>
          <OrderPicker teams={teams} teamOrder={teamOrder} value={ordMode} onChangeOrd={handleOrd} prevOrder={teamOrder}/>
          <button onClick={()=>onNext(ordVal)} style={{width:"100%",padding:"14px 0",border:"none",borderRadius:10,background:"#14365a",color:"#fff",fontSize:17,fontWeight:700,cursor:"pointer",marginTop:4}}>次のゲーム開始</button>
        </div>)}
        {isAllDone&&(<div style={{background:"#fff",borderRadius:12,padding:14,marginBottom:10,border:"1px solid #d0dff0"}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:4}}>
            <button onClick={()=>onExtend("game",ordVal)} style={{flex:1,padding:"12px 0",border:"none",borderRadius:8,background:"#2b7de9",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>＋1ゲーム追加</button>
            {isMatchOver&&<button onClick={()=>onExtend("set",ordVal)} style={{flex:1,padding:"12px 0",border:"none",borderRadius:8,background:"#22b566",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>＋1セット延長</button>}
          </div>
        </div>)}
        <button onClick={onBack} style={{width:"100%",padding:"14px 0",border:"2px solid #14365a",borderRadius:10,background:"transparent",color:"#14365a",fontSize:17,fontWeight:700,cursor:"pointer",marginBottom:20}}>設定に戻る</button>
      </div>
    </div>
  );
}
...
function GameScreen({initialTeams,initialOrder,bestOf:iBo,numGames:iNg,dqEnd,goBack}){
  ...
  const cpIdx=ap.length>0?td%ap.length:0;

  /* Active cell for blinking */
    ...
    <div style={SS.gW}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}>
        ...
        {(view==="both"||view==="input")&&(
          <ScoreInput dispatch={dispatch} canUndo={history.length>0} teamName={teams[ti].name} teamScore={score} teamColor={C[ti].ac} playerName={cp?.name} fails={fails} onConfirm={(t,s,m)=>setConf({t,s,msg:m})} minimized={inputMin} onToggleMin={()=>setInputMin(p=>!p)}/>
        )}
      </div>
