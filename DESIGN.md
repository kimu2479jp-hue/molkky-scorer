...
**`--text-danger` 背景流用の暫定許容（PR β、2026-04-25）**:

実コード調査（PR β 事前調査、2026-04-25）で `--text-danger` を `background` プロパティに流用しているボタンが 6 ヒット / 実 UI 10 個確認された（パターン2、削除確認モーダル系）。これは本セクションの正典定義「`--text-danger` は文字色専用」の用途逸脱に該当する。

PR β（Danger hover 実装）では、用途逸脱の修正を行わず**現状を暫定許容**する方針を採択した。これは以下の判断による:

- PR β の本来スコープは「Danger hover の CSS クラス導入による正典化」であり、トークン用途の整理は別軸の課題
- 用途逸脱を修正するには 6 箇所のボタン背景を別トークン（`--danger` ベース or 新トークン）に置き換える必要があり、視覚変化の影響評価が別途必要
- 暫定許容により PR β は「`--text-danger-hover` トークンの新設 + `.mk-btn-danger-strong` クラス」という形でこの用途逸脱を**正規の hover 対応として包摂する**構造を取る

用途逸脱の本格整理（パターン2 を `--danger` 系統への統合 or 別の Semantic 新設）は、別タスク **「状態色トークン体系第3次再設計」**（§2.10.3 末尾参照）で扱う。
...
Apple Watch Ultra Industrial 美学（Wayfinder 文字盤 + Modular Ultra 文字盤参照）の WindMonitorModal 専用 16 トークン。PR β X-2（PR #91、2026-04-24）で `src/styles.css` に 12 トークン追加、PR β X-3（PR #92、2026-04-24）で `src/components/WindMonitorModal.jsx` に全面適用、PR 1（2026-04-26、WindMonitor 美学棚卸し）で `--wind-battery-null` / `--wind-battery-mid` / `--wind-border-subtle` / `--wind-shadow-deep` の 4 トークンを追加し残余 hex 直書き 15 箇所を解消、PR 2（2026-04-28、WindMonitor 美学棚卸し完結）でレンジゲージ STRONG 帯 `#f87171` を `--wind-strong` に統合し残余 hex 棚卸しタスク完結。既存の `--wind-bg-base` / `--wind-bg-card` / `--wind-grid-*` / `--wind-text-*` の 6 トークン定義（2025 年時点）は本節の書き換えで廃止され、新 16 トークン体系に置き換わる。
...
**派生タスク（PR β で発生、2026-04-25）**:

- **状態色トークン体系第3次再設計**: `--text-danger` を `background` プロパティに流用しているボタン 6 ヒット / 実 UI 10 個（パターン2、削除確認モーダル系）を本格整理する。§2.4 の用途逸脱（暫定許容）を解消するため、以下のいずれかの方針で実施する想定:
  - 案 A: パターン2 を `--danger` 系統に統合し、`--text-danger-hover` および `.mk-btn-danger-strong` を廃止
  - 案 B: パターン2 専用の新 Semantic（例: `--danger-strong` / `--danger-strong-hover`）を新設し、`--text-danger` の用途を文字色専用に戻す
  - 案 C: 現状の暫定許容構造を恒久化し、§2.4 の正典定義を「`--text-danger` は文字色 + 削除系強調背景の両用」に拡張

  実施タイミングは PR γ / δ / ε / ζ 完了後、状態色トークン全体の最終整合段階で判断する。詳細は §2.4「`--text-danger` 背景流用の暫定許容」副節参照。

**派生タスク（PR γ で発生、2026-04-25）**:

- **CHA 定数の borderColor 単独化（PR δ で対応）**: `src/components/SetupScreen.jsx:334` の CHA 定数は PR γ で `background` を削除して `borderColor: "var(--blue-500)"` のみを保持する形に整理された。PR δ Secondary hover 実装時に、CHA の `borderColor` を `.mk-btn-secondary` クラス（仮称）に統合する形で再整理する。
...
- 2026-04-25: `--danger-hover` および `--text-danger-hover` を新設（PR β、派生：§ 7.2 Buttons 全面実装、Danger hover 実装第 1 弾）
- 2026-04-25: `.mk-btn-primary` クラスを追加（PR γ、派生：§ 7.2 Buttons 全面実装、Primary hover 実装、新規トークン追加なし）
...
| hover | 背景 `--blue-700`（PC マウス環境のみ、CSS クラス `.mk-btn-primary` で適用） |
| active | （未実装、将来 PR ζ で確定） |
| disabled | （未実装、将来 PR ζ で確定） |
| focus | （未実装、将来 PR ζ で確定） |

**hover 実装方式（PR γ、2026-04-25 確定）**: CSS クラス `.mk-btn-primary` を `src/styles.css` に追加する方式で確定。`@media (hover: hover) and (pointer: fine)` ガードで PC マウス環境のみ hover 反応を有効化し、タッチデバイス（iPad / iPhone）での誤発動を防止する。`transition: background var(--transition-normal)` を `:hover` 内に閉じ込めることで、通常時のクリック反応や状態変化への影響を回避する。hover 時の背景色は既存の `--blue-700`（PR α で `:root` に正典化済み、§ 2.3 Brand Blue 参照）を使用し、新規トークン追加なし。

**実装メモ**: PR γ では shadow 深化（`--shadow-sm` → `--shadow-md`）は実装しない。デザイン正典としては保留扱いとし、将来 PR ζ で focus / disabled / active / shadow 深化を一括整備する際に再検討する。

**使用制限**: 主要操作・確定アクション・選択中状態・タブのアクティブ表現にのみ使用。Toggle Switch ON 色（§ 7.4 参照）や Input focus border（§ 7.4 参照）は同じ `--blue-500` を使用するが、`.mk-btn-primary` クラスの適用対象外（hover 反応不要）。
...
| hover | 背景 `--danger-hover`（PC マウス環境のみ、CSS クラス `.mk-btn-danger` で適用） |
| active | `transform: scale(0.98)` |
| disabled | （未実装、将来 PR ζ で確定） |
| focus | （未実装、将来 PR ζ で確定） |

**hover 実装方式（PR β、2026-04-25 確定）**: `--danger-hover: color-mix(in srgb, var(--danger) 85%, black)` を新設し、CSS クラス `.mk-btn-danger` を `src/styles.css` に追加する方式で確定。`@media (hover: hover) and (pointer: fine)` ガードで PC マウス環境のみ hover 反応を有効化し、タッチデバイス（iPad / iPhone）での誤発動を防止する。`transition: background var(--transition-normal)` を `:hover` 内に閉じ込めることで、通常時のクリック反応や状態変化への影響を回避する。同時に新設された `--text-danger-hover` と `.mk-btn-danger-strong` は、ボタン背景に `--text-danger` を流用している既存箇所（§2.4 暫定許容注記参照）への hover 適用に使用する。

**実装メモ**: 第2弾B-β 当時に保留されていた「具体値（`--danger-hover` 新設 or `--danger-dark` 再定義）」は前者（`--danger-hover` 新設）で確定。`--danger-dark` 再定義案は採択せず、`--danger-hover` を新規 Semantic として独立保持する。
