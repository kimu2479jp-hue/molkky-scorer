...
DESIGN_AUDIT.md で「定義されているが使用箇所数 0」と分類されたもの。Step 4 第2弾以降で削除・統合・定義値変更の対象。

| 変数名 | 現状値 | DESIGN.md マップ先 | Step 4 第1弾の扱い | 第2弾以降の計画 |
|---|---|---|---|---|
| `--bg-overlay-light` | `rgba(0,0,0,0.4)` | なし | 現状維持 | 未使用維持 or 削除判断 |
| `--accent-red` | `#d93a5e` | → `--danger` (`#e74c3c`) | 現状維持 | 定義値変更 `#d93a5e` → `#e74c3c`（視覚変化あり） |
| `--accent-gold` | `#ffd700` | `--gold` | 現状維持 | `--gold` への統合 |
| `--border-focus` | `#2b7de9` | `--blue-500` | 現状維持 | `--blue-500` への統合 |
| `--radius-sm` | `6px` | → `8px`（§6 参照） | 現状維持 | 定義値変更 `6px` → `8px`（視覚変化あり） |
| `--radius-md` | `10px` | §6 要確認 | 現状維持 | §6 Radii との整合性を第2弾で確認 |
| `--radius-lg` | `14px` | → `12px`（§6 参照） | 現状維持 | 定義値変更 `14px` → `12px`（視覚変化あり） |
| `--radius-xl` | `18px` | → `16px`（§6 参照） | 現状維持 | 定義値変更 `18px` → `16px`（視覚変化あり） |
| `--shadow-md` | `0 6px 20px rgba(0,0,0,0.18)` | §5 要確認 | 現状維持 | §5 Shadows との整合性を第2弾で確認 |
| `--transition-fast` | `0.15s ease` | §8 要確認 | 現状維持 | §8 Motion との整合性を第2弾で確認 |
| `--transition-normal` | `0.2s ease` | §8 要確認 | 現状維持 | §8 Motion との整合性を第2弾で確認 |
| `--transition-slow` | `0.3s ease` | §8 要確認 | 現状維持 | §8 Motion との整合性を第2弾で確認 |
...
| 要素 | スタイル |
|---|---|
| Overlay | `rgba(0,0,0,0.5)`、`backdrop-filter: blur(4px)` 任意 |
