...
| `--text-danger` | `#c0392b` | 30 | `--danger-dark` | 現状維持 | 値一致 |
...
| `--text-warning` | `#e67700` | 1 | `--warning` に近似（微差） | 現状維持 | `--warning` (`#e6a817`) と微妙に異なる。Step 4 第2弾で統合判断 |
...
| `--border-lighter` | `#eee` | → `--neutral-100` | 現状維持 | `--neutral-100` への統合（第2弾B） | - |
| `--border-light` | `#e0e0e0` | → `--neutral-200` | 現状維持 | `--neutral-200` への統合（第2弾B） | - |
| `--border-input` | `#ddd` | → `--neutral-200` | 現状維持 | `--neutral-200` への統合（第2弾B） | - |
...
現状のモーダル実装では `style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.55)'}}` 形式の直書きが中心であり、`var(--bg-overlay)` は実コードで参照されていない。本変数は将来的な統一用途として保持する。
...
- **風速計エラー時**: 同位置に「風速計NG」バッジを代替表示（`--danger-bg` + `--danger-dark`）
...
| フォルト | `--danger-bg` / `--danger-dark` | 違反・失格 |
...
- 1〜10点 + ミス: プレイヤー色ソリッド
- **11〜12点のみゴールド強調**: `--gold`（派手すぎる場合は `--stats-highlight-gold` #c9a227 を新トークンとして追加可、実装時判断）
  - 意味論: 「上がり得点の輝き」を演出
