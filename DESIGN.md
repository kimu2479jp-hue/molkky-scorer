...
#### 2.11.4 既知の課題（将来リファクタ対象）

現状、Semantic トークンは Primitive を `var()` 参照せず直接値を記述している。

例: `--text-primary: #14365a;` であり、`--text-primary: var(--neutral-800);` ではない。

このため Primitive 側の値を変更しても Semantic には自動反映されず、保守性が低い。`var()` 参照化への書き換えは視覚変化を伴わない機械的リファクタだが、§2.11.2 の表に掲げた 10 組すべてを一括処理する必要があり、影響範囲が広い。2-c では実施せず、**将来の独立タスクとして積み残し**とする。この独立タスクでは §2.11.2 注記で言及した 3 桁／6 桁 hex 表記ゆれも同時に統一する。
